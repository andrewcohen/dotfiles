...
- ALWAYS pass `-m "..."` to `jj squash` (e.g. `jj squash --from <rev> --into <rev> -m "..."`). Squashing two described commits otherwise opens an editor to combine their messages; supply the intended combined message inline instead of relying on the default concatenation.
